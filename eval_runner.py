...
class EvaluationRunner:
    ...
    def list_available_models(self) -> Dict[str, List[str]]:
        ...
        for provider, config in self.config.items():
            if 'models' in config and config.get('api_key'):
    ...
    async def run_evaluation(
        ...
        for i, test_case in enumerate(test_suite["test_cases"], 1):
            ...
            evaluation = self.evaluator.evaluate_response(response, test_case)
        ...
        # Save results
        results_file = self.storage.save_evaluation_results(results, model_name)
...
async def main():
    ...
    # Initialize runner
    runner = EvaluationRunner(base_path)
    
    # Set up argument parser
    parser = argparse.ArgumentParser(description='Run LLM evaluations with various models and test suites.')
    ...
    if not results_files:
        if not args.list:
            ...
            print("\nUsage examples:")
            print("  python eval_runner.py --list")
            print("  python eval_runner.py --provider openai --model gpt-3.5-turbo --test-suite sample_test_suite.json")
            print("  python eval_runner.py --all-models --test-suite sample_test_suite.json")
        return
